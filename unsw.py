...
result_data = []
# Metadata features: srcip, sport, dstip, dsport, service, stime, ltime
metadata_feature = 'ltime'

for k,v in unsw_data.items():
    dataset = k   
    ...
    if metadata_feature == 'srcip' or metadata_feature == 'dstip' or metadata_feature == 'service':
        df[metadata_feature] = df[metadata_feature].astype('category').cat.codes
    elif metadata_feature == 'stime' or metadata_feature == 'ltime':
...
result_df.to_csv(f'results/unsw-{metadata_feature}.csv', header=True, index=False, encoding='utf-8')
print(result_df)
